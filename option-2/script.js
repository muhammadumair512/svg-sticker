...
function startGradientEffect() {
  const gradientElements = document.querySelectorAll("#gradient1 stop");
  const svgElement = document.querySelector("#svglogo");

  function handleDeviceTilt(x, y) {
    // Normalize tilt values and scale sensitivity
    const normalizedX = Math.max(-1, Math.min(1, x / 45)); // [-1, 1]
    const normalizedY = Math.max(-1, Math.min(1, y / 45)); // [-1, 1]
    ...
    // Dynamic gradient stop offsets
    const offsets = [
      Math.min(100, Math.max(0, 10 + normalizedX * 20)),
      Math.min(100, Math.max(0, 30 + normalizedY * 20)),
      Math.min(100, Math.max(0, 50 + normalizedX * 20)),
      Math.min(100, Math.max(0, 70 + normalizedY * 20)),
      100,
    ];

    // Update gradient colors
    gradientElements.forEach((stop, index) => {
  ...
  window.addEventListener("deviceorientation", (event) => {
    const tiltX = event.beta || 0; // Front/back tilt
    const tiltY = event.gamma || 0; // Left/right tilt
    handleDeviceTilt(tiltX, tiltY);
  });
}
