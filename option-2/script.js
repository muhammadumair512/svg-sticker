...
function startGradientEffect() {
  ...
  startMotionHandler((x, y) => {
    ...
    const neonOffsetY = Math.round(Math.sin((angle * Math.PI) / 180) * 15);

    const neonColor = {
      r: 0 + Math.round((1 + normalizedX) * 127), // Red range: 0-255
      g: 255 - Math.round((1 + normalizedY) * 127), // Green range: 128-255
      b: 255 - Math.round((1 - normalizedX) * 127), // Blue range: 128-255
    };

    const neonGlowColor = `rgba(${neonColor.r}, ${neonColor.g}, ${neonColor.b}, 1)`;
    const shadowGlow = `${neonOffsetX}px ${neonOffsetY}px 20px ${neonGlowColor}, 0px 0px 30px ${neonGlowColor}`;

    // Apply dynamic shadow to neon elements
    neonElements.forEach((element) => {
      element.style.filter = `drop-shadow(${shadowGlow})`;
    });

    // Apply dynamic shadow to SVG logo
